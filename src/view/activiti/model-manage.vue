...
export default {
  ...
  data() {
    return {
      ...
      },
      {
        ...
        align: 'center',
        render(h, params) {
          return h('span', formatDate(params.row.lastUpdateTime, 'yyyy-MM-dd hh:mm'))
        },
      },
      {
        title: '操作',
  ...
  methods: {
    ...
    showModel(params) {
      this.modal = true
      this.modelerUrl = `//127.0.0.1:8080/modeler.html?modelId=${params.row.id}&time=${new Date().getTime()}`
    },
    handAdd() {
