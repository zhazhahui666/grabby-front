  ...
  <div>
    <row>
      ...
      <card>
        ...
        <row class="row">
          ...
          名称：
          <Input v-model="searchForm.name" placeholder="请输入名称" style="width: auto"
        ...
        <!-- 数据表格 -->
        <row>
          <Table border highlight-row @on-current-change="handCurrentChange"
            :columns="columns" :data="dataList"></Table>
        </row>

        <!-- 分页 -->
        <row type="flex" justify="end" class="page">
          <Page size="small" :total="totalCount" show-elevator show-sizer
          ...
          <Tabs v-model="tabSelect" @on-click="handTabClick">
            ...
            <TabPane label="功能权限" icon="logo-windows" name="permission">
              <row v-if="permissionTab.dataList && permissionTab.dataList.length">
                <Button type="primary" class="action-btn" icon="md-people"
                  @click="savePermission">保存修改</Button>
                <!-- <Col span="12" v-for="item in permissionTab.dataList" :key="item.id">{{item.name}}</Col> -->
                <Tree :data="permissionTab.dataList" ref="permissionTree"
                  show-checkbox></Tree>
              </row>
              <row v-else>暂无数据</row>
            </TabPane>
            <TabPane label="成员列表" icon="md-person" name="people">
              <row class="row">
                <Button type="primary" class="action-btn" icon="md-person-add"
                  @click="userAddModalShow">添加成员</Button>
              </row>
              <!-- 数据表格 -->
              <row>
              ...
              <!-- 分页 -->
              <row type="flex" justify="end" class="page">
                <Page :total="userBlock.totalCount" size="small" show-elevator
                  show-sizer show-total @on-change="changeUserPage"
    ...
    <Modal v-model="roleModal.show" title="角色添加" @on-ok="addRoleOk" loading>
      <Form :model="currentRole" :label-width="80">
        ...
        <FormItem label="描述：">
          <Input v-model="currentRole.description" type="textarea" autosize
            style="width: 300px"></Input>
        </FormItem>
      </Form>
    </Modal>

    <Modal v-model="deleteRoleModal.show" title="角色删除" @on-ok="deleteRoleOk"
      loading>
      <span>确定要删除「{{currentRole.name}}」角色吗，删除后,数据将不可恢复</span>
    </Modal>

    <Modal v-model="userAddModal.show" title="成员添加" @on-ok="addUserOk" loading
      okText="添加">名称：
      <Input v-model="userAddModal.searchForm.username" placeholder="请输入名称"
        style="width: auto" @on-enter="handSearch"></Input>
      <Button type="primary" @click="handUserModaloSearch" icon="md-search">搜索</Button>
      <Divider style="margin:12px 0" />
      <!-- 数据表格 -->
      <row>
        <Table border :columns="userAddModal.columns" :data="userAddModal.dataList"
          @on-selection-change="userModalSelectChange"></Table>
      </row>
      <!-- 分页 -->
      <row type="flex" justify="end" class="page">
        <Page :total="userAddModal.totalCount" size="small" show-elevator
          show-sizer show-total @on-change="changeUserModalPage"
          @on-page-size-change="changeUserModalPageSize" />
      </row>
    </Modal>
  </div>
</template>
