      ...
      <Row>
        ...
        <div class='content-wrapper'>
          <div class="content-header">
            <span @click="changeContent('department')">部门信息</span>
            <span @click="changeContent('people')">人员信息</span>
            <Button type="primary">人员导入</Button>
            <Button type="primary" @click="userModal = true">添加人员</Button>
            <Button type="primary">新建同级部门</Button>
            <Button type="primary">新建下级部门</Button>
          </div>
          <div class="content-body">
            <div v-if="contentShow == 'department'">
              <p>{{department.id}}</p>
              <p>{{department.name}}</p>
            </div>
...
export default {
  ...
  data() {
    return {
      ...
      contentShow: 'department', //people||department
      columns: [{
        title: '用户名',
        key: 'username'
      },
  ...
  methods: {
    ...
    transferData2Tree(data) {
      if (data.length > 0) {
        return data.map(({ pid, id, name, children }) => ({
          id,
          title: name,
